...
long scanEstSecBat(volatile char *str) {
	...
	do {
		...
		pIn++;								// next input char
	} while ((str[pIn] != NULL) && (pIn < strlen((char *)str)));	// leave if done
	return(sec);
}

void scanParams(volatile struct upsDataStrucT *upsData) {

	volatile int msgPos, paramPos, paramNum;
	...
	if (strncmp("^0",(char *)upsData->msgStr,2) != 0) {		// returns 0 if same, so a non-zero means the ups understood command
		...
		while ( (upsData->msgStr[msgPos] != NULL) && (msgPos < TEMP_BUFFER_LENGTH) ){
			...
			if (msgChar == ',') {						// comma delimited list
				...
				if (paramPos < (PARAM_LEN_MAX-2)) {		// check for overrun, leave room for next char and null
					upsData->StrParams[paramNum][paramPos++] = msgChar;
					// Null will move until last char put into string insuring string terminated with null
					upsData->StrParams[paramNum][paramPos] = NULL;
				}
				else {
				}
			}
...
void ups_com(volatile struct upsDataStrucT *upsData)
{
		...
		if (usart_rx_buffer_count(upsData->port)) {	// char in buffer between router sourced cmds
			...
			if ((masterCmdPending(pUps)) && (timer(upsData->timeOutStart, 250))) {
				...
				if ( (timer(upsData->timeCmdMade, UPS_POLL_INTERVAL)) &&
					...
					if (++upsData->timedCmd > (sizeof(upsCmdRotation)/sizeof(int) - 1)) { // this works, I tested it.
						upsData->timedCmd = 0;
					}
	...
	case COM_WAITING:	// waiting for incoming msg to complete
		...
		if (timer(upsData->timeOutStart, 500)) {
			sprintf((char *)eventStrTemp,"ups_com:WAITING, Com Error, Message Timeout for [%s], ups %d"
		...
		case COM_WAIT_MESSAGE:
			...
			if (usart_rx_buffer_count(upsData->port)) {	// if char pending, get it
				...
				if (upsData->msgChr == 10) {			// newline shows end of data, ready for checksum
					...
					if ( (upsData->msgChr >= 20) && (upsData->msgChr < 127) ) {	// only store printable chars
						...
						if (upsData->nextChrPos >= TEMP_BUFFER_LENGTH) {	// too big
			...
			if (usart_rx_buffer_count(upsData->port)) {	// if char pending, get it
				...
				if (upsData->msgChr == 10) {			// newline shows end of data, ready for checksum
					...
					if ( (upsData->msgChr >= '0') && (upsData->msgChr <= '9') ) {// only store numbers
						...
						if (upsData->nextTempChrPos >= TEMP_BUFFER_LENGTH) {	// too big
							upsData->upsComState = COM_IDLE;					// stop, dump and get next one
						}
					}
				}
	...
	case COM_RESPONSE:	// Not used for timed updating, this is when question is first asked by Upsilon
		...
		if (upsData->checksumMode == CHKSUM_ON) {
			...
			for (i=0;i<strlen((char *)upsData->msgStr);i++) {
		...
		} else {																// no checksum check, message passes
			upsData->comOkay = TRUE;
		}
		if (upsData->comOkay == TRUE) {
			...
			case RDAT01_PARAMS:													// parameter 0 is header
				...
				pString = &upsData->StrParams[10][0];
				...
				pString = &upsData->StrParams[11][0];
				...
				pString = &upsData->StrParams[12][0];
				upsData->voltBat = atof((char *)pString);
				#if DUAL_BOARD==FALSE											// not ganged Model F boards
				...
				pString = &upsData->StrParams[13][0];
				upsData->ampBat = atof((char *)pString);
					...
					pString = &upsData->StrParams[14][0];
			...
			case RDAT02_PARAMS2:
				...
				pString = &upsData->StrParams[1][0];								// parameter 0 is header
			...
			case RDAT06_MISC_PARMS:
				...
				if (upsBoss.batSts != upsBoss.batStsLast) {
				...
				// in ups chg_status 1=fast, 2=slow, 3=off, reports => fast=1, slow=0, other=2
				pString = &upsData->StrParams[3][0];
				upsData->chgModeSnmp = atoi((char *)pString);
				#if defined THALES_3KVA
					upsBoss.chgModeSnmp = upsTwo.chgModeSnmp;			// upsTwo is the Model F charger
				#else
					#if DUAL_BOARD==FALSE								// single board
				...
				#endif
				// Inverter Fault
				pString = &upsData->StrParams[7][0];		// output bad alarm
				...
				//	pString = &upsData->StrParams[8][0];	// UPS Software version
				...
				#endif
				if (strlen((char *)&upsData->StrParams[10][0]) < IDENT_LEN_MAX) {
					strcpy((char *)upsData->man,(char *)&upsData->StrParams[10][0]);
				}
				if (strlen((char *)&upsData->StrParams[11][0]) < IDENT_LEN_MAX) {
					strcpy((char *)upsData->model,(char *)&upsData->StrParams[11][0]);
				}
				break;
			#if defined UCLASS_FA10002
			...
			case RDAT31_UPS_PROG_VER:			// Get UPS Master and Slave Firmware Version
				...
				if (strlen((char *)&upsData->StrParams[1][0]) < IDENT_LEN_MAX) {
					strcpy((char *)upsData->verSoftware,(char *)&upsData->StrParams[1][0]);
				} else {
					upsData->comOkay = FALSE;
					#if DUAL_BOARD==FALSE											// single board
...
void snmp_com(volatile struct upsDataStrucT *upsData, volatile struct snmpDataStruct *parseType)
{
	...
	case SNMP_WAITING:	// waiting for incoming msg to complete
		...
		if ((usart_peekchar(parseType->snmpPort) == '^') && (parseType->pos > 0)) {
			...
			if (usart_rx_buffer_count(parseType->snmpPort)) {									// if char pending, get it
				if (parseType->pos >= SNMP_STR_MAX) {						// str buffer full
					parseType->type = FAIL;								// dump message, start again
				} else {	// str buffer not full
					// get char, put in string, bump pointer
					parseType->str[parseType->pos++] = parseType->snmpChar = usart_getchar(parseType->snmpPort);
					...
					case DATA:
						...
						if (parseType->subPos < (SNMP_STR_MAX-1)) {						// at string max - 2, add char and null
							...
							if (parseType->subPos >= parseType->dataLen) {				// when position+1 = length, stop
								parseType->str[0] = NULL;									// reset, used for part of response msg
	...
	case SNMP_RESPONSE:	// Not used for timed updating, this is when question is first asked by Upsilon
		...
		case AP1:		// Poll commands, some Set commands, variables implemented by us
			...
			strcpy((char *) parseType->str,"7,8,10,14,16,17,18,19,20,22,24,36,37,38,39,40,43,46");
		...
		case AP2:
			...
			strcpy((char *) parseType->str,"47,51,54,55,56,58,59,60,61,62,63,65,68,69,72,73,76,77,80,82,83,84,85,88,89");
			// sprintf will take the message, put ^D in front, then 3 digit length, then msg ex. "^D0052,230"
			parseType->dataLen = strlen((char *) parseType->str);
			sprintf((char *) parseType->responseStr,"^D%03d%s", parseType->dataLen,(char *) parseType->str);
			usart_putstr(parseType->responseStr,parseType->snmpPort);
			parseType->snmpComState = SNMP_IDLE;
			break;
		case ATR:		// Autostart Poll and Set
			if (upsData->bank1 & BIT1) {	// see if autoStart bit is set
				strcpy((char *) parseType->str,"1");
			}
			else {
				strcpy((char *) parseType->str,"2");
			}
			// sprintf will take the message, put ^D in front, then 3 digit length, then msg ex. "^D0052,230"
			parseType->dataLen = strlen((char *) parseType->str);
			strcpy((char *) parseType->responseStr,"^D001");
			strcat((char *) parseType->responseStr,(char *) parseType->str);
			usart_putstr(parseType->responseStr,parseType->snmpPort);
			parseType->snmpComState = SNMP_IDLE;
			break;
		case MAN:		// Manufacturer
			strcpy((char *) parseType->str,(char *) upsData->man);
			// sprintf will take the message, put ^D in front, then 3 digit length, then msg ex. "^D0052,230"
			parseType->dataLen = strlen((char *) parseType->str);
			...
			break;
		case MOD:		// Model
			//strcpy((char *) parseType->str,(char *) upsData->model);
			strcpy((char *) parseType->str,UPS_MODEL);
			parseType->dataLen = strlen((char *) parseType->str);
			//	sprintf(parseType->responseStr,"^D%03d%s",parseType->dataLen, parseType->str);
			strcpy((char *) parseType->responseStr,"^D");
			strcat((char *) parseType->responseStr,itoa3(parseType->dataLen));
		...
		case NOM:	// Nominal values for parameters
			...
			//	strcpy(parseType->str,"230,600,230,600,5000,4000,1,2,170,280");
			strcpy((char *) parseType->str,itoa((int) upsBoss.voltInNom));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa((int) upsBoss.freqInNom*10));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa((int) upsBoss.voltOutNom));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa((int) upsBoss.freqOutNom*10));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa((int) upsBoss.vaOutNom));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa((int) upsBoss.powOutNom));
			#if (defined SNMP_NOM_LOW_BAT_TIME)
				strcat((char *) parseType->str,SNMP_NOM_LOW_BAT_TIME);
			#else
				strcat((char *) parseType->str,",1");
			#endif
			...
			// Override low/high input voltage transfer in configuration file
				...
				strcat((char *) parseType->str,SNMP_NOM_INPUT_TRANSFER);
			#else
				strcat((char *) parseType->str,",100,140");
			#endif
			parseType->dataLen = strlen((char *) parseType->str);
			//	sprintf(parseType->responseStr,"^D%03d%s", parseType->dataLen,parseType->str);
			strcpy((char *) parseType->responseStr,"^D");
		...
		case PSD:		// Timed Shutdown, Set command unique
			if (parseType->type == POLL) {
				strcpy((char *) parseType->str,itoa(upsBoss.secShutdownDelay));
				parseType->dataLen = strlen((char *) parseType->str);
				strcpy((char *) parseType->responseStr,"^D");
				strcat((char *) parseType->responseStr,itoa3(parseType->dataLen));
				strcat((char *) parseType->responseStr,(char *) parseType->str);
				...
				#endif
				ltemp1 = atol((char *) &parseType->StrParams[0][0]);
		...
		case STD:		// Timed Startup, Set command unique
			if (parseType->type == POLL) {
				strcpy((char *) parseType->str,itoa(upsBoss.secStartupDelay));
				parseType->dataLen = strlen((char *) parseType->str);
				strcpy((char *) parseType->responseStr,"^D");
				strcat((char *) parseType->responseStr,itoa3(parseType->dataLen));
				strcat((char *) parseType->responseStr,(char *) parseType->str);
		...
		case ST1:
			...
			/*
			sprintf(parseType->str,"%d,%d,%d,%05d,%d,%d,%d,%d,%d",
			...
			*/
			...
			strcpy((char *) parseType->str,itoa(temp2));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa(upsBoss.batSts));
			strcat((char *) parseType->str,",");
			// in ups chg_status 1=fast, 2=slow, 3=off, reports => fast=1, slow=0, other=2
			strcat((char *) parseType->str,itoa(upsData->chgModeSnmp));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa((int) upsData->secOnBat));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa((int) (upsData->estSecBat/60.0) + 0.5));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa(temp1));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa((int) ((upsData->voltBat + 0.5)*10)));
			strcat((char *) parseType->str,",");
			#if !defined UCLASS_FA10002
				if ((upsBoss.dcMode == AUTO_ON) || (upsBoss.dcMode == MANUAL_ON)) {
					strcat((char *) parseType->str,itoa((int) ((upsData->ampBat + 0.5)*10)));
				} else {
					strcat((char *) parseType->str,"0");		// not on batteries, zero current
				}
			#else												// defined UCLASS_FA10002
				strcat((char *) parseType->str,itoa((int) (upsData->ampBat*10)));
			#endif
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa((int) (upsData->tAmb+0.5)));
			parseType->dataLen = strlen((char *) parseType->str);
			//	sprintf(parseType->responseStr,"^D%03d%s", parseType->dataLen,parseType->str);
			strcpy((char *) parseType->responseStr,"^D");
			strcat((char *) parseType->responseStr,itoa3(parseType->dataLen));
		...
		case ST2:
			...
			//	sprintf(parseType->str,",1,%d,%d",
			//		(int) (upsData->freqIn*10),		// average 60.3=>603
			//		(int) (upsData->voltIn*10));		// output in series so add together
			strcpy((char *) parseType->str,",1,");		// Number of input lines
			strcat((char *) parseType->str,itoa((int) (upsOne.freqIn*10)));
			strcat((char *) parseType->str,",");
			#ifdef CRYSTAL_OLD							// Original Crystal can't sense input voltage
				if (upsBoss.upsState == UPS_ON_UTIL) {
			...
			#endif
			strcat((char *) parseType->str,itoa((int) (upsOne.voltIn*10)));
			strcat((char *) parseType->str,",,,");											// input current and power
			strcat((char *) parseType->str,itoa((int) (upsTwo.freqIn*10)));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa((int) (upsTwo.voltIn*10)));
			parseType->dataLen = strlen((char *) parseType->str);
			strcpy((char *) parseType->responseStr,"^D");
			strcat((char *) parseType->responseStr,itoa3(parseType->dataLen));
			strcat((char *) parseType->responseStr,(char *) parseType->str);
		...
		case ST3:
			...
			case UPS_OFF:
				...
				// and then if there is overload bypass. The unit is in bypass but upsState is UPS_OFF				
					...
					if (upsBoss.bypassMode == ON) {
						strcpy((char *) parseType->str, "2");	// in bypass
					} else {
						strcpy((char *) parseType->str, "5");	// "Other"
					}
					break;									// don't drop down to UPS_INIT
				#endif
			case UPS_INIT:									// UPS on "Other"
				strcpy((char *) parseType->str, "5");
				break;
			case UPS_ON_BAT:
			case UPS_SHUTDOWN:
				strcpy((char *) parseType->str, "1");		// UPS on battery
				break;
			case UPS_ON_UTIL:
				if (upsBoss.bypassMode == ON) {			// ON, OFF or AUTO
					strcpy((char *) parseType->str, "2");	// UPS on bypass
				} else if ((upsOne.dcMode == AUTO_ON) || (upsTwo.dcMode == AUTO_ON)) {
					strcpy((char *) parseType->str, "1");	// UPS on battery (shutting down)
				} else {
					strcpy((char *) parseType->str, "0");	// UPS on utility "normal"
				}
				break;
			...
			//	sprintf(parseType->responseStr,"%s,%d,1,%d,%d,%d,%d",
			//		parseType->str,(int)(upsData->freqOut*10),
			//		(int)((upsData->voltOut + 0.5)*10),
			//		temp1,(int)(upsData->powOut + 0.5),temp2);
			// ST3-1 Output Source
			strcpy((char *) parseType->responseStr,(char *) parseType->str);			// output source, set above
			strcat((char *) parseType->responseStr,",");
			// ST3-2 Output Frequency
			strcat((char *) parseType->responseStr,itoa((int)(upsData->freqOut*10)));
			// ST3-3 Output number of lines
			strcat((char *) parseType->responseStr,",1,");
			// ST3-4 Output Voltage 1
			strcat((char *) parseType->responseStr,itoa((int)((upsData->voltOut + 0.5)*10)));
			strcat((char *) parseType->responseStr,",");
			// ST3-5 Output Current 1
			strcat((char *) parseType->responseStr,itoa(temp1));
			strcat((char *) parseType->responseStr,",");
			// ST3-6 Output Power 1
			if (upsBoss.bypassMode == ON) {			// if either board on bypass
				strcat((char *) parseType->responseStr,itoa((int) 0 ));	// no power out
			} else {
				strcat((char *) parseType->responseStr,itoa((int)(upsData->powOut + 0.5)));
			}
			strcat((char *) parseType->responseStr,",");
			// ST3-7 Output Load 1
			strcat((char *) parseType->responseStr,itoa(temp2));
			strcpy((char *) parseType->str,(char *) parseType->responseStr);
			// CORES2 extended MIB
			/*
			strcat(parseType->responseStr,",");
			// ST3-8 Output Voltage 2
			strcat(parseType->responseStr,itoa((int)((upsOne.voltOut + 0.5)*10)));
			strcat(parseType->responseStr,",");
			// ST3-9 Output Current 2
			strcat(parseType->responseStr,itoa((int)((upsOne.ampOut + 0.5)*10)));
			strcat(parseType->responseStr,",");
			// ST3-10 Output Power 2
			strcat(parseType->responseStr,itoa((int)(upsOne.powOut + 0.5)));
			strcat(parseType->responseStr,",");
			// ST3-11 Output Load 2
			strcat(parseType->responseStr,itoa((int)((((upsOne.powOut/2400.0) * 100) + 0.5)*10)));
			strcat(parseType->responseStr,",");
			// ST3-8 Output Voltage 3
			strcat(parseType->responseStr,itoa((int)((upsTwo.voltOut + 0.5)*10)));
			strcat(parseType->responseStr,",");
			// ST3-9 Output Current 2
			strcat(parseType->responseStr,itoa((int)((upsTwo.ampOut + 0.5)*10)));
			strcat(parseType->responseStr,",");
			// ST3-10 Output Power 2
			strcat(parseType->responseStr,itoa((int)(upsTwo.powOut + 0.5)));
			strcat(parseType->responseStr,",");
			// ST3-11 Output Load 2
			strcat(parseType->responseStr,itoa((int)((((upsTwo.powOut/2400.0) * 100) + 0.5)*10)));
			*/			
			parseType->dataLen = strlen((char *) parseType->str);
			strcpy((char *) parseType->responseStr,"^D");
			strcat((char *) parseType->responseStr,itoa3(parseType->dataLen));
		...
		case ST4:	// Bypass information
			//	strcpy((char *) parseType->responseStr, "^D001,");	// send message that feature is not supported
			// message above was used to prevent a problem with the SNMP
			// now using the unknown command response
			strcpy((char *) parseType->responseStr, "^D001,");	// send message that feature is supported but send nothing
			usart_putstr(parseType->responseStr,parseType->snmpPort);
			parseType->snmpComState = SNMP_IDLE;
			break;
		case ST5:
			...
			if ((upsBoss.loadPctOut >= 105.0) || (upsTwo.loadPctOut >= 105.0)) {
				strcat((char *) parseType->str, "1,");		// Overload Alm = Inverter fault, ST5-4
			}
			else {
				strcat((char *) parseType->str, "0,");
			}
			strcat((char *) parseType->str, ",");			// Bypass bad alarm, not implemented, ST5-5
			if ((upsBoss.invMode == AUTO_OFF) || (upsBoss.invMode == MANUAL_OFF)) {
				strcat((char *) parseType->str, "1");		// Output Bad Alm = Inverter off, ST5-6
			}
			else {
				strcat((char *) parseType->str, "0");
			}
			/*
			strcat((char *) parseType->str, ",");				// UPS Shutdown, not implemented, ST5-7
			strcat((char *) parseType->str, ",");				// Charger Failure, not implemented, ST5-8
			strcat((char *) parseType->str, ",");				// System Off, not implemented, ST5-9
			...
			strcat((char *) parseType->str, ",");				// Fuse Failure, not implemented, ST5-11
			...
			strcat((char *) parseType->str, ",");				// Awaiting Power, not implemented, ST5-13
			strcat((char *) parseType->str, ",");				// Shutdown Pending, not implemented, ST5-14
			strcat((char *) parseType->str, ",");				// Shutdown Imminent, not implemented, ST5-15
			strcat((char *) parseType->str, "0,");				// PDU Status, ST5-16
			...
			strcat((char *) parseType->str,itoa(upsOne.batSts));// Battery1 Status, ST5-19
			strcat((char *) parseType->str, ",");
			strcat((char *) parseType->str,itoa(upsTwo.batSts));// Battery2 Status, ST5-20
			strcat((char *) parseType->str, ",");
		...
		case UBR:		// Poll and Set, Baud rate 1200, 2400, 4800, 9600, 19200
			strcpy((char *) parseType->str, "9600");
			parseType->dataLen = strlen((char *) parseType->str);
			//	sprintf(parseType->responseStr,"^D%03d%s", parseType->dataLen,parseType->str);
			strcpy((char *) parseType->responseStr,"^D");
			strcat((char *) parseType->responseStr,itoa3(parseType->dataLen));
		...
		case UID:		// UPS Identification, if you had more than one you could number them 1-xx
			...
			if (parseType->type == POLL) {
				...
				strcpy((char *) parseType->str,itoa(upsBoss.snmpUid));
				parseType->dataLen = strlen((char *) parseType->str);
				strcpy((char *) parseType->responseStr,"^D");
				strcat((char *) parseType->responseStr,itoa3(parseType->dataLen));
				strcat((char *) parseType->responseStr,(char *) parseType->str);
				...
				strcpy((char *) parseType->responseStr,"^1");
				usart_putstr(parseType->responseStr,parseType->snmpPort);
				flashPointer = (unsigned int *) 0x1800;		// start of Seg D
				flashPointer += 2;								// 0x1800 and 0x1801 used by Joule Pool
		...
		case VER:
			if (strlen((char *) upsData->verSoftware) < (SNMP_STR_MAX-1)) {
				strcpy((char *) parseType->str, (char *) upsData->verSoftware);
			} else {
				strcpy((char *) parseType->str, "Unknown, string length error");
			}
			//	strcat(parseType->str, ", PC1 ");
			//	strcat(parseType->str, upsOne.verSoftware);
			//	strcat(parseType->str, ", PC2 ");
			//	strcat(parseType->str, upsTwo.verSoftware);
			parseType->dataLen = strlen((char *) parseType->str);
			//	sprintf(parseType->responseStr,"^D%03d%s", parseType->dataLen,parseType->str);
			strcpy((char *) parseType->responseStr,"^D");
			strcat((char *) parseType->responseStr,itoa3(parseType->dataLen));
		...
		case XD1:
			...
			strcpy((char *) parseType->str,itoa((int) ((upsData->ampChg + 0.5)*10)));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa(temp[1]));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa(temp[2]));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa((int) upsData->pfOut));
			strcat((char *) parseType->str,",");
			strcat((char *) parseType->str,itoa(temp[4]));
			parseType->dataLen = strlen((char *) parseType->str);
			//	sprintf(parseType->responseStr,"^D%03d%s", parseType->dataLen,parseType->str);
			strcpy((char *) parseType->responseStr,"^D");
			strcat((char *) parseType->responseStr,itoa3(parseType->dataLen));
		...
		case XD2:
			/*	// code from charger_com()
				...
				pString = &parseType->StrParams [1][0];
				upsOne.ampChg = atof((char*) pString);
				//pString = &parseType->StrParams [2][0];
				//upsOne.ampBat = atof((char*) pString);
				//pString = &parseType->StrParams [3][0];
				//upsTwo.voltBat = atof((char*) pString);
				pString = &parseType->StrParams [4][0];
				upsTwo.ampChg = atof((char*) pString);
				//pString = &parseType->StrParams [5][0];
				//upsTwo.ampBat = atof((char*) pString);
			*/
			if (parseType->type == POLL) {
				sprintf((char *) parseType->str, "%1.2f,%1.3f,%1.2f,%1.2f,%1.3f,%1.2f,%ld,%ld"
				...
				//upsTwo.ampBat = atof((char*) pString);
				#if (defined CHARGER_MONSTER)					// Dual Model F charger
					pString = &parseType->StrParams [6][0];
					upsOne.batJoule = atol((char*) pString);
					pString = &parseType->StrParams [7][0];
		...
		// after responses have finished, print to monitor
			...
			usart_putstr(parseType->responseStr, COMM_MONITOR_PORT); // send string to indicated port
			usart_putstr("\r\n", COMM_MONITOR_PORT); // send string to indicated port
		#endif										// defined COMM_MONITOR_PORT
		break;
	default:
...
void ups_state_controller(void)
{
	...
	//	__disable_interrupt();	// TODO - interrupt disable ups_state_controller
	DEBUG_PORT1_1;	// #################
	...
	case UPS_INIT:											// Initial state when router/system powers up
		...
		case 1:
			...
			// TODO - set to 12000 for normal, 60000 for devel
	...
	case UPS_ON_BAT:									// On battery, inverter on
		...
		if (upsBoss.voltBat <= upsBoss.vBatShutdown) {
		#endif
		...
		if (delayedShutdown == TRUE) {					// delayed shutdown active
			...
			} else if (timer(delayedShutdownTime, (long) upsBoss.secShutdownDelay * 1000)) {
	...
	case UPS_OFF:												// System powered but Inverter off
		if (upsBoss.upsState != upsBoss.lastUpsState) {		// state entry code
			...
			} else {											// Overload Trip = FAULT condition
		...
		default:
			...
			*/
			...
			if (delayedStartup == TRUE) {					// delayed startup active
				...
				} else if (timer(delayedStartupTime, (long) upsBoss.secStartupDelay * 1000)) {
...
*/
	...
	void batCapJouleUpdate(volatile struct upsDataStrucT *upsData) {
			...
			if (systemStart == FALSE) {
				...
				if (upsStartup == TRUE) {
					upsStartup = FALSE;							// just do this once on startup
				...
				if ( ((upsOne.dcMode == AUTO_ON) || (upsOne.dcMode == MANUAL_ON))
					&& ((upsOne.invMode == AUTO_OFF) || (upsOne.invMode == MANUAL_OFF))
					&& (upsShutdown == FALSE) ) {
					upsShutdown = TRUE;							// do this once when shutting down
					// save joules to flash
					FlashPointer = (long *) 0x1800;			// start of Seg D
					__disable_interrupt();						// 5xx Workaround: Disable global
					// interrupt while erasing. Re-Enable
			...
			if (upsData->powOut >= 50.0) {
				upsData->estSecBat = upsData->batJoule / upsData->powOut;	// calculate remaining runtime in seconds
			} else {											// don't divide by zero
				upsData->estSecBat = upsData->batJoule / 50.0;// calculate remaining runtime in seconds
			}
			#if ((!defined THALES_3KVA) || (DUAL_BOARD==TRUE))
				upsBoss.estSecBat = lMin(upsOne.estSecBat,upsTwo.estSecBat);	// min
			#else // defined THALES_3KVA
...
#ifdef COM_ETI													// compiler switch for ETI data and functions
//*****************************************************************************************
...
// ETI definitions
	...
	struct etiDataStruct {
		...
		bool fIF, fOB, fLB, fCH, fBP;						// Boolean Flags for state of Input, On Battery, Low Battery, Charging, and Bypass
	...
	void initETIcom (volatile struct etiDataStruct *parseType) {
		...
		parseType->posRCV = 0;
		parseType->fIF = parseType->fOB = parseType->fLB = FALSE;	// Preset conditions to FALSE
		parseType->fCH = parseType->fBP = FALSE;
	...
	char etiStatusCheck (volatile struct upsDataStrucT *upsData, volatile struct etiDataStruct *parseType) {
		...
		if (batteryTrayStatus) {																				// Battery Tray Pulled check
			parseType->fIF = parseType->fOB = FALSE;															// Set Input Fault and not on battery (because there is none)
			parseType->fLB = TRUE;																				// Set Low Battery Alarm
			...
			#endif
			// If upsState and fIF are different set flag. Update fIF Status
			test = parseType->fIF;								// Do we think we are on Utility?
			...
			if (test1 ^= test) {								// If we disagree, then update ourself and set update flag
				flag = TRUE;
				parseType->fIF = parseType->upsUtilityOK;
			}													// Exclusive-OR the results... a difference means update
			// If upsState and fOB are different set flag. Update fOB Status
			test = parseType->fOB;								// Do we think we are on Battery?
			test1 = parseType->upsBatteryOps;					// Is the system on Battery?
			if (test1 ^= test) {								// If we disagree, then update ourself and set update flag
				flag = TRUE;
				parseType->fOB = parseType->upsBatteryOps;
			}													// Exclusive-OR the results... a difference means update
			// If batSts and fLB are different set flag. Update fLB Status
			test = parseType->fLB;								// Do we think we are Low Battery?
			test1 = upsData->batSts == 1;						// Is the system Low Battery?
			if (test1 ^= test) {								// If we disagree, then update ourself and set update flag
				flag = TRUE;
				parseType->fLB = (upsData->batSts == 1);
			}													// Exclusive-OR the results... a difference means update
			// Are we in any Charge Mode and are running on Utility?
			// If chgMode and fCH are different set flag. Update fCH Status
			test = parseType->fCH;								// Do we think we are Charging?
			#if DUAL_BOARD
			...
			parseType->fCH = test1;								// Because XOR is destructive
			if (test1 ^= test) {								// If we disagree, then update ourself and set update flag
				flag = TRUE;
			}													// Exclusive-OR the results... a difference means update
			// If UPS_BYPASS and sBP are different set flag. Update fBP Status
			test = parseType->fBP;								// Do we think we are on Bypass?
			test1 = upsData->bypassMode == ON;					// Is the system on Bypass?
			if (test1 ^= test) {								// If we disagree, then update ourself and set update flag
				flag = TRUE;
				parseType->fBP = (upsData->bypassMode == ON);
			}													// Exclusive-OR the results... a difference means update
			tempBatPer = upsData->batChgPct * 10.0;																	// Re-scale
			parseType->pctBat = (int) (tempBatPer + 0.5);																	// Load the value into the memory structure for reporting
			...
			if (parseType->upsBatteryOps) {																			// Are we on Batteries?
				...
				batTimeValue = (long) (60.0 * tempBatPer * UPS_TIMELOBATNOM / 100.0);								// No, so we may be charging so base the use on Battery Charge Percent and UPS Battery Time
			}																										// End of on Battery adjustment check and setting of batTimeValue
			...
			firstTray = TRUE;																					// Tray not pulled so we get to be virgin again
	...
	void extendedResponses (volatile struct upsDataStrucT *upsData, volatile struct etiDataStruct *parseType) {
		...
		case EXTENDED_BATTERY:																// Extended Status parameters
			...
			case OVER_VOLTAGE:
				...
				break;
			case FAULT:
				strcat ((char *) parseType->response, "FALUT");
				break;
			default:
		...
		strcat ((char *) parseType->response, "\r\n");										// <CR> terminate the string for passage
	...
	void ETIcom (volatile struct upsDataStrucT *upsData, volatile struct etiDataStruct *parseType) {
		...
		if ((parseType->BatteryAlarmTest) && (timer(parseType->timeBatteryTest, testTimeBatteryAlarmRelay))) {
			...
			#if defined ETI_VERBOSE
				if (!parseType->fLB) {
					strcpy ((char *) parseType->response, "Battery Alarm Relay - Test Override set OFF. End of Test.\r\n");
					masterCmdAdd ("^RBUT23", &upsOne);
				} else {
					strcpy ((char *) parseType->response, "Battery Alarm Relay - End of Test - Alarm is real!\r\n");
				}
		...
		#if defined ETI_VERBOSE
			if (parseType->fIF) {
				strcpy ((char *) parseType->response, "General Alarm Relay - Test Override set OFF. End of Test.\r\n");
				masterCmdAdd ("^RBUT21", &upsOne);
			} else {
				strcpy ((char *) parseType->response, "General Alarm Relay - End of Test - Alarm is real!\r\n");
			}
			usart_putstr (parseType->response, parseType->Port);
			strcpy ((char *) parseType->response, NULL);															// Reset communication parameters
		#else
		...
		case ETI_RESPONSE:
			...
			case GENERAL_ALARM_RELAY_NORMAL:
				#if defined ETI_VERBOSE
					if (parseType->fIF) {																			// If AC Input OK then action, else skip because it is real
						parseType->GeneralAlarmTest = FALSE;
						masterCmdAdd ("^RBUT21", &upsOne);
						strcpy ((char *) parseType->response, "General Alarm Relay - Normal Operation\r\n");
					} else {
						strcpy ((char *) parseType->response, "General Alarm Relay - Is in alarm state!\r\n");
					}
					usart_putstr (parseType->response, parseType->Port);
				#else
					if (parseType->fIF) {																			// If AC Input OK then action, else skip because it is real
			...
			case GENERAL_ALARM_RELAY_TEST:
					...
					if (parseType->fIF) {																			// If AC Input OK then action, else skip because it is real
						...
						strcpy ((char *) parseType->response, "General Alarm Relay - Test Override set ON for 3 minutes\r/n");
					} else {
						strcpy ((char *) parseType->response, "General Alarm Relay - Is in alarm state!\r\n");
					}
					usart_putstr (parseType->response, parseType->Port);
			...
			case BATTERY_ALARM_RELAY_NORMAL:
				#if defined ETI_VERBOSE
					if (!(parseType->fLB)) {																		// IF not Low Battery then action, else it is real
						parseType->BatteryAlarmTest = FALSE;
						masterCmdAdd ("^RBUT23", &upsOne);
						strcpy ((char *) parseType->response, "Battery Alarm Relay - Normal Operation\r\n");
					} else {
						strcpy ((char *) parseType->response, "Battery Alarm Relay - Is in alarm state!\r\n");
					}
					usart_putstr (parseType->response, parseType->Port);
				#else
					if (!(parseType->fLB)) {																		// IF not Low Battery then action, else it is real
			...
			case BATTERY_ALARM_RELAY_TEST:
					...
					if (!(parseType->fLB)) {																		// IF not Low Battery then action, else it is real
						...
						strcpy ((char *) parseType->response, "Battery Alarm Relay - Test Override set ON for 3 minutes\r\n");
					} else {
						strcpy ((char *) parseType->response, "Battery Alarm Relay - Is in alarm state!\r\n");
					}
					usart_putstr (parseType->response, parseType->Port);
...
//*****************************************************************************************
	...
	void init_charger_com (volatile struct snmpDataStruct *parseType) {
		parseType->str[0] = parseType->aLen[0] = parseType->aData[0] = NULL;	// NULL them out
	...
	void charger_com (volatile struct upsDataStrucT *upsData, volatile struct snmpDataStruct *parseType) {
		...
		volatile char ccData [SNMP_STR_MAX];
		static volatile int comFailures = 0;
		...
		case SNMP_BYPASS:
			if (parseType->snmpComState != parseType->lastSnmpComState) {	// State entry code
				...
				switch (chargerData.cCmd) {
				case cST1:
					usart_putstr ("^P003ST1", parseType->snmpPort);			// Poll for the data
					chargerData.cCmd = cXD1;									// Next command please
					chargerData.cLastCmd = ST1;								// Set our override command
					break;
				case cXD1:
					usart_putstr ("^P003XD1", parseType->snmpPort);			// Poll for the data
					chargerData.cCmd = cXD2;									// Next command
					chargerData.cLastCmd = XD1;								// Set our override command
					break;
				case cXD2:
					usart_putstr ("^P003XD2", parseType->snmpPort);			// Poll for the data
					#if (!defined CHARGER_MONSTER)								// Normal operation
						chargerData.cCmd = cCMD_END;							// Next command
						chargerData.cLastCmd = XD2;							// Set our override command
					#else														// Sends XD3 Set command
						chargerData.cCmd = cSXD2;								// Next command
						chargerData.cLastCmd = XD2;							// Set our override command
					#endif
					break;
				#if (defined CHARGER_MONSTER)									// Sends XD3 Set command
					case cSXD2:													// Set instead of Poll XD2
						sprintf((char *) parseType->str, "XD2%1.2f,%1.3f,%1.2f,%1.2f,%1.3f,%1.2f,%ld,%ld"
							,upsOne.voltBat,upsOne.ampChg,upsOne.ampBat,upsTwo.voltBat,upsTwo.ampChg,upsTwo.ampBat
							,upsOne.batJoule,upsTwo.batJoule);
						parseType->dataLen = strlen((char *) parseType->str);
						strcpy((char *) parseType->responseStr,"^S");
						strcat((char *) parseType->responseStr,itoa3(parseType->dataLen));
						strcat((char *) parseType->responseStr,(char *) parseType->str);
						usart_putstr(parseType->responseStr,parseType->snmpPort);
						chargerData.cCmd = cCMD_END;							// Next command
				...
				case cCMD_END:
				default:
					chargerData.cCmd = cST1;									// Restart the command polling
					//chargerData.cLastCmd = NO_SNMP_CMD;						// Set our override command
					break;} // End of commands
				parseType->snmpComState = SNMP_IDLE;							// Now watch for communications
			}	// End state entry code
			break; // End of SNMP_BYPASS
		case SNMP_IDLE:	// Waiting for timed command, state change cmd or ups generated notification (button press, etc.)
			if (parseType->snmpComState != parseType->lastSnmpComState) {	// state entry code
				parseType->lastSnmpComState = parseType->snmpComState;
				//parseType->timeOutStart = getTime();
		...
		case SNMP_RESPONSE:	// Not used for timed updating, this is when question is first asked by Upsilon
			...
			case ST1:
				pString = &parseType->StrParams [0][0];
				switch ((int) atof ((char*) pString)) {
				case 0:
					upsData->batSts = NORMAL;
					break;
				case 1:
					upsData->batSts = WARN;
					break;
				case 2:
					upsData->batSts = OVER_VOLTAGE;	// Tricky double on the other side
					break;
				default:
					upsData->batSts = FAULT;
					break;}
				pString = &parseType->StrParams [1][0];
				upsData->batSts = atoi ((char*) pString);
				pString = &parseType->StrParams [2][0];
				...
				pString = &parseType->StrParams [5][0];
				if (parseType->parser == SNMP) {upsData->batChgPct = atof ((char*) pString);}
				else {upsData->batChgPct = (atof ((char*) pString) / 10.0 - 0.5);}
				upsData->batChgPct = atof ((char*) pString);
				pString = &parseType->StrParams [6][0];
				upsData->voltBat = (atof ((char*) pString) / 10 - 0.5);
				pString = &parseType->StrParams [7][0];
				upsData->ampBat = (atof ((char*) pString) /10 - 0.5);
				pString = &parseType->StrParams [8][0];
				upsData->tAmb = atof ((char*) pString);
				parseType->snmpComState = SNMP_IDLE;
				parseType->type = OKAY;
				break;
		...
		if (timer (chargerData.timePollCharger, chargerData.pollingTime)) {
			if ( (parseType->type == UNKNOWN) || (parseType->type == FAIL) ) {
				if (comFailures++ > 30) {
					comFailures = iMin (comFailures, 40);					// Limit value so it doesn't roll over
					parseType->comOkay = FALSE;
				}
			}
			parseType->snmpComState = SNMP_BYPASS;							// Expired? Yes = do a poll
		}
...
void main(void){
	...
	// **debug
	static volatile struct timeT timeTemp, timeGot;
	// **debug end
	#if BAT_CAP_METHOD==BAT_CAP_JOULE
	...
	*/
	...
	Forever {	//main loop
		...
		if (timer(timeStarted, 1000)) {
			...
			if (timer(timeUpsStateRunning, 1000)) {// wait until we have information from master and slave UPS
		#else
		...
		if (timer(timeVersionChecked, 15000)) {		// time to check
			...
			if (timer(timeComLed, 100)) {		// turned on in UART interrupt red=xmit green=rcv
